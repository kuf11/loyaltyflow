...
async function synchronizeUser(user){const token=decryptSecret(user.bot_token_enc,jwtSecret),url=appUrl(user.id),text=String(user.miniapp_design?.menuButtonText||user.miniapp_design?.name||'Открыть приложение').trim().slice(0,64)||'Открыть приложение',menu_button={type:'web_app',text,web_app:{url}};await telegram(token,'setChatMenuButton',{menu_button});await telegram(token,'setMyCommands',{commands:[{command:'start',description:text}]});let synchronized=0;for(const chat_id of await customerChats(user)){try{await telegram(token,'setChatMenuButton',{chat_id,menu_button});synchronized++}catch(error){console.error('Bot chat menu sync failed',user.bot_username||user.id,chat_id,error.message)}await sleep(60)}console.log('Bot menu synchronized',user.bot_username||user.id,'chats:',synchronized)}
