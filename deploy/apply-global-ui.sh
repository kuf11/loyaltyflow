...
sed -i -E 's#cookie-ui\.js\?v=[0-9]+#cookie-ui.js?v=5#g;s#mobile-audit\.css\?v=[0-9]+#mobile-audit.css?v=3#g;s#session-stability\.js\?v=[0-9]+#session-stability.js?v=2#g' "$SITE"
...
if ! grep -q 'mobile-audit.css' "$SITE"; then sed -i 's#<script src="/cookie-ui.js?v=5"></script>#<link rel="stylesheet" href="/mobile-audit.css?v=3"><script src="/cookie-ui.js?v=5"></script>#g' "$SITE"; fi
...
grep -q 'mobile-audit.css?v=3' "$SITE" || { echo 'Global mobile CSS injection failed' >&2; exit 1; }
nginx -t
