...
'''
    if marker not in text: raise SystemExit('Could not find HTTPS API location in Nginx config')
    path.write_text(text.replace(marker,block+marker,1))
PY
nginx -t
systemctl reload nginx
curl --retry 10 --retry-connrefused --retry-delay 2 -fsS http://127.0.0.1:3101/admin-api/health
echo
