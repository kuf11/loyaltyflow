...
server { listen 443 ssl http2; listen [::]:443 ssl http2; server_name ${DOMAIN}; ssl_certificate /etc/letsencrypt/live/${DOMAIN}/fullchain.pem; ssl_certificate_key /etc/letsencrypt/live/${DOMAIN}/privkey.pem; include /etc/letsencrypt/options-ssl-nginx.conf; ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; root /opt/loyaltyflow; index index.html; client_max_body_size 8m; sub_filter_once on; sub_filter '</head>' '<script src="/session-cookie.js?v=4"></script></head>'; location = /session-cookie.js { add_header Cache-Control "no-store, no-cache, must-revalidate" always; try_files \$uri =404; } location ~ /\.(?!well-known) { deny all; } location ^~ /deploy/ { deny all; } location = /docker-compose.yml { deny all; } location ~* \.(?:sql|sh|md|ya?ml|lock)$ { deny all; } location /api/ { proxy_pass http://127.0.0.1:${API_PORT}/api/; proxy_http_version 1.1; proxy_set_header Host \$host; proxy_set_header X-Forwarded-Proto https; proxy_set_header X-Real-IP \$remote_addr; proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for; } location /admin-api/ { proxy_pass http://127.0.0.1:${ADMIN_API_PORT}/admin-api/; proxy_http_version 1.1; proxy_set_header Host \$host; proxy_set_header X-Forwarded-Proto https; proxy_set_header X-Real-IP \$remote_addr; proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for; } location / { try_files \$uri \$uri/ /index.html; } add_header X-Content-Type-Options nosniff always; add_header X-Frame-Options SAMEORIGIN always; add_header Referrer-Policy strict-origin-when-cross-origin always; add_header Strict-Transport-Security "max-age=31536000; includeSubDomains" always; add_header Permissions-Policy "camera=(), microphone=(), geolocation=()" always; add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' https://telegram.org https://cdnjs.cloudflare.com https://cdn.jsdelivr.net https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; img-src 'self' data: https:; connect-src 'self' https://api.telegram.org https://challenges.cloudflare.com; frame-src 'self' https://challenges.cloudflare.com; object-src 'none'; base-uri 'self'; frame-ancestors 'self' https://web.telegram.org https://*.telegram.org" always; }
NGINX
nginx -t; systemctl reload nginx
sleep 5
curl -fsS http://127.0.0.1:${API_PORT}/api/health
curl -fsS http://127.0.0.1:${ADMIN_API_PORT}/admin-api/health
curl -ksS --resolve "${DOMAIN}:443:127.0.0.1" "${SCHEME}://${DOMAIN}/" | grep -q 'session-cookie.js?v=4'
curl -ksS --resolve "${DOMAIN}:443:127.0.0.1" "${SCHEME}://${DOMAIN}/session-cookie.js" | grep -q 'Render cookie notice\|essential-cookie-notice'
