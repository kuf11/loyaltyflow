...
Итог передаётся в `LoyaltyFlowActivity` как `EXTRA_RECEIPT_TOTAL`. В интерфейсе сумма показывается автоматически и недоступна для ручного редактирования. При проверке QR она также выводится в статусе. Вводить сумму покупки вручную не нужно.
...
Баланс из QR нельзя считать достоверным. Окончательное решение должно приниматься сервером, а операции должны быть идемпотными.
